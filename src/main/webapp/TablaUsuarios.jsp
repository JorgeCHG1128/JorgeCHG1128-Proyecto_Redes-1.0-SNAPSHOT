<%-- 
    Document   : TablaUsuarios
    Created on : 3 abr. 2024, 11:53:45
    Author     : bporr
--%>

<%@page contentType="text/html" pageEncoding="UTF-8"%>
<%@ page import="java.util.List" %>
<%@ page import= "model.Usuario" %>
<!DOCTYPE html>
<html>
    <head>
        <link rel="stylesheet" href="css/Tablas.css">
        <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.10.0/font/bootstrap-icons.css">
        <meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
        <title>Usuarios</title>
    </head>
    <body>
        <div class="menu">
            <ul>
                <li><a href="TablaUsServlet">Tabla Usuarios</a></li>
                <li><a href="TablaPeServlet">Tabla Películas</a></li>
                <li><a href="TablaProServlet">Tabla Productos</a></li>
                <li></li>
                <li></li>
                <li></li>
                <li><a href="Login.jsp">Cerrar Sesión</a></li>
            </ul>
        </div>

        <div class="contenido-principal">
            <a href="TablaUsServlet?action=agregar" title="Agregar Usuarios"><i class="bi bi-plus-circle"></i></a>
            <a href="TablaUsServlet?action=ver" title="Refrescar Lista"><i class="bi bi-arrow-clockwise"></i></a>
            <h1>Lista Usuarios!</h1>
            <table>
                <tr>
                    <th>ID_Usuario</th><th>Email</th><th>Password</th><th>Rol</th><th>Acciones</th>
                </tr>

                <% 
                    List<Usuario> listaUsuarios = (List<Usuario>) session.getAttribute("listaUsuarios");
                    for (Usuario usuario : listaUsuarios) {
                %>
                <tr>
                    <td><%= usuario.getID_Usuario() %></td>
                    <td><%= usuario.getEmail() %></td>
                    <td><%= usuario.getPassword() %></td>
                    <td><%= usuario.getRol() %></td>
                    <td>
                        <%--
                       <a href="EstudianteServlet?action=view&id=<%= estudiante.getId() %> " title="Vista"><i class="bi bi-binoculars"></i></i></i></a>
                       <a href="EstudianteServlet?action=editar&id=<%= estudiante.getId() %> " title="Editar"><i class="bi bi-pencil"></i></i></a>
                        --%>
                       <a href="TablaUsServlet?action=eliminar&id=<%= usuario.getID_Usuario() %> " title="Eliminar"><i class="bi bi-trash3"></i></a>

                    </td>
                </tr>
                <% } %>

            </table>
        </div>
    </body>
</html>
